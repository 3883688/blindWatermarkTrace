...
* **体积贴近源图**：在 quality 88~92 区间里挑**能塞进体积预算的最高质量**，
  既不让成品明显大于原图，又尽量少丢信息。
...
# quality 下限 88：继续降低会明显加大 DCT 码字被量化削弱的风险。
# 上限 92：更高质量会快速增加体积，对肉眼画质和水印存活率的收益有限。
JPEG_MIN_QUALITY = 88
JPEG_MAX_QUALITY = 92
# 体积预算 = 源图字节数 × 1.10。留 10% 余量容纳水印引入的额外高频
# 细节，同 quality 下必然比原图大一些；把预算卡死在 100% 会逼着质量无谓地往下掉。
JPEG_TARGET_RATIO = 1.10
...
def encode_adaptive_jpeg(
    ...
    **从高质量往低走**是关键：第一个塞得进预算的就返回，所以拿到的是"预算内的
    ...
    整个区间都超预算时，返回 88 这一档并接受体积超标——**水印可靠性下限比体积
    约束更硬**。继续降低会明显增加 DCT 域码字被量化削弱的风险。
