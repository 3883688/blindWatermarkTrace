...
**Goal:** Add a Chinese root README that accurately presents Trace System for developers, operators, and open-source readers.

**Architecture:** Keep the root README as the project navigation document: it describes the modular FastAPI application and links to the existing deployment guide for production detail. Commit the approved existing `demo/1.jpg` unchanged and reference it by repository-relative Markdown path.

**Tech Stack:** Markdown, Git, FastAPI, Uvicorn, SQLite/MySQL configuration

---

## File Structure

- Create: `README.md` - Chinese project overview, architecture, local development guide, API map, and production deployment link.
- Add: `demo/1.jpg` - existing approved 3840x2160 project demo image, committed without modification.
- Modify: no application source, configuration, deployment, or test files.

### Task 1: Create the Root Project Guide

**Files:**
- Create: `README.md`
- Add: `demo/1.jpg`

- [ ] **Step 1: Confirm the README and demo asset are absent from tracked files**

Run:

```powershell
git ls-files -- README.md demo/1.jpg
Get-Item demo/1.jpg | Select-Object FullName, Length
```

Expected: `git ls-files` has no output; the asset exists at `demo/1.jpg`.
...
- [ ] **Step 3: Validate paths, Markdown structure, and staged scope**

Run:

```powershell
@('README.md','README_DEPLOY.md','demo/1.jpg','trace_app/api') | ForEach-Object {
    if (-not (Test-Path $_)) { throw "Missing README reference: $_" }
}
if ((Select-String -Path README.md -Pattern '^# ' | Measure-Object).Count -ne 1) {
    throw 'README must contain one H1 heading'
}
git diff --check
git status --short
```

Expected: every referenced path exists, README has one H1 heading, no whitespace errors, and only `README.md`, `demo/1.jpg`, and documentation files are selected for this feature.

- [ ] **Step 4: Commit the README and approved demo asset**

```powershell
git add -- README.md demo/1.jpg
git commit -m "docs: add project README"
```

Expected: the commit contains exactly the root README and `demo/1.jpg`; runtime data and unrelated images remain untracked.
