...
def _snapshot_roles(engine: Engine) -> tuple[tuple[Any, ...], ...]:
    with engine.connect() as connection:
        return tuple(
            connection.execute(
                text("SELECT role_key, label FROM roles ORDER BY role_key")
            ).all()
        )


def _snapshot_menus(engine: Engine) -> dict[str, tuple[str, ...]]:
    ...
    if "menus" in role_columns:
        with engine.connect() as connection:
            rows = connection.execute(
                text("SELECT role_key, menus FROM roles ORDER BY role_key")
            ).all()
...
def migrate(engine: Engine) -> None:
    ...
    users_before = _snapshot_users(engine)
    roles_before = _snapshot_roles(engine)
    menus_before = _snapshot_menus(engine)
    ...
    with engine.begin() as connection:
        ...
        for role_key, menus in menus_before.items():
            for position, menu_key in enumerate(menus):
                connection.execute(
                    text(
                        "INSERT INTO role_menus(role_key, menu_key, position_index) "
