...
describe('watermark form parity', () => {
  test('retains every legacy watermark control and its defaults', () => {
  ...
  test('creates the exact legacy embed FormData field names', () => {
    ...
    expect([...data.entries()].map(([key, value]) => [key, value instanceof File ? value.name : value])).toEqual([
      ['file', 'source.png'], ['user_id', 'BX100'], ['mode', 'dct'],
