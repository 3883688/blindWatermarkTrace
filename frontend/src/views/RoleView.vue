...
async function load() { try { const config = await listRoles(); roles.value = config.roles || {}; menus.value = config.menus || {}; selected.value = Object.fromEntries(Object.entries(roles.value).map(([key, role]) => [key, [...(role.menus || [])]])); } catch (error) { showAlert(error.message); } }
async function save(key) { try { const config = await saveRole(key, selected.value[key] || []); roles.value = config.roles || {}; menus.value = config.menus || {}; selected.value[key] = [...(roles.value[key]?.menus || [])]; } catch (error) { showAlert(error.message); } }
onMounted(load);
</script>
